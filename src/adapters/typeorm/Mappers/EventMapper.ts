...
export const EventMapper = {
  toDomain(raw: Event): EventEntity {
    return new EventEntity({
      id: raw.id,
      name: raw.name,
      sub_title: raw.sub_title ? raw.sub_title : '',
      ...
      projects: raw.projects ? raw.projects.map(ProjectMapper.toDomain) : [],
      categories: raw.categories
        ? raw.categories.map(CategoryMapper.toDomain)
        : [],
      forms: raw.forms ? raw.forms.map(EvaluationFormMapper.toDomain) : [],
      reviewers: raw.reviewers
        ? raw.reviewers.map(EvaluationReviewerMapper.toDomain)
        : [],
      attendance_sessions: raw.attendance_sessions
  ...
  toSchema(raw: EventEntity): Event {
    ...
    event.id = raw.id
    event.name = raw.name
    event.sub_title = raw.sub_title ? raw.sub_title : ''
    ...
    event.projects = raw.projects
      ? raw.projects.map(ProjectMapper.toSchema)
      : []
    event.categories = raw.categories
      ? raw.categories.map(CategoryMapper.toSchema)
      : []
    event.forms = raw.forms ? raw.forms.map(EvaluationFormMapper.toSchema) : []
    event.reviewers = raw.reviewers
      ? raw.reviewers.map(EvaluationReviewerMapper.toSchema)
      : []
    event.attendance_sessions = raw.attendance_sessions
